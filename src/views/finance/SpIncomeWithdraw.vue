    ...
    <el-dialog title="详情" :visible.sync="withdrawDialog">
      ...
      <div v-if="withdrawInfo.status == 1" class="opera-btn">
        <el-button type="primary" @click="confirmRefound(2)"
          >确认提现</el-button
        >
        <el-button type="danger" @click="confirmRefound(3)">拒绝提现</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    exportWithdrawList() {
      window.location.href =
        BASEURL +
        "/admin/pay/sponsor/v1/income/audit_list_export?status=" +
        this.search.status +
        "&start_time=" +
    ...
    async withDrawSearch() {
      ...
      let res = await this.$api.incomeWithdrawList(this.$initReq(req));
      if (res.error_code == 0) {
